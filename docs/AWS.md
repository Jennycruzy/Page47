...
| Amazon SESv2 | Send one review email to each matching watch. | The sender and recipient code is implemented, duplicate sends are recorded, and each message includes the private watch-management link. AWS Support approved production access in `eu-west-2` on 15 September 2026. A VPS check on 17 September found the sender parameter present, `SendingEnabled=true`, `ProductionAccessEnabled=true`, `Max24HourSend=50000`, `MaxSendRate=14`, and `VerifiedForSendingStatus=true`; the account is out of the SES sandbox. Controlled application delivery and mailbox receipt were already verified on 10 September while sandboxed. | Residents do not receive a review by email if SES is unavailable or delivery is misconfigured. |
