...
| Amazon SESv2 | Send one review email to each matching watch. | The sender and recipient code is implemented, duplicate sends are recorded, and each message includes the private watch-management link. The 9 September 2026 VPS check found the sender parameter present, `SendingEnabled=true`, `ProductionAccessEnabled=false`, and `VerifiedForSendingStatus=true`. No notification row or controlled message has been recorded yet; the sandbox recipient/receipt test remains. | Residents do not receive a review without logging into the console. |
| Page 47 console service | Serve the resident console and API from the public host. | `page47-web.service` is enabled and healthy on `127.0.0.1:8090`. DNS, the `page47.xcover.online` certificate, dedicated Nginx host, HTTP redirect, HTTPS health, root response, renewal dry-run, root-relative links, legacy `/page47` redirect, and the public URL parameter are verified. | Notification links cannot be tested until the controlled SES delivery is complete. |
...
4. Configure the SES sender parameter and verify SES sender status. **Partial:** the sender parameter and verified identity are present; a controlled message and receipt remain.
