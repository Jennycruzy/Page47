...
| AgentCore Runtime | Host the bursty five-role review graph separately from the always-on Lightsail service. | The installed `bedrock-agentcore` 1.22.0 package and an entrypoint are in the repository. `ListAgentRuntimes` returned an empty list; no managed runtime has been created. | The graph remains a local library call and cannot be shown as the managed AWS workload required for the live demonstration. |
| Systems Manager Parameter Store | Hold the SES sender address and the public service URL outside source control. | Delivery code expects `/page47/email/sender` and `/page47/web/public-url`; these values have not been installed or verified yet. | Email delivery fails loudly instead of exposing a secret or sending a message from an unknown address. |
...
1. Create the least-privilege AgentCore execution role with Bedrock invocation and trace permissions, then create the runtime from the checked-in entrypoint. Keep `iam:PassRole` limited to that role.
2. Add an AgentCore client in the Lightsail API. It must send the stored case, captured document bytes, and their SHA-256 values to the runtime, then apply the deterministic review rule on the service side.
3. Configure the two SSM parameters and verify SES sender status. Send one real message to a controlled address and retain the provider result without logging credentials.
4. Put the console behind a real domain and HTTPS, then verify it in a clean browser.
