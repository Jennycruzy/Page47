...
| Page 47 console service | Serve the resident console and API from Lightsail. | `page47-web.service` is enabled and healthy on `127.0.0.1:8090`. Nginx serves it over the existing certificate at `https://xcover.online/page47/`; direct health, city, ledger, and proxied API checks returned 200. | Residents and judges cannot browse the live records, watch setup, findings, or captured documents. |
