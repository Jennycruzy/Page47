...
| AgentCore Runtime | Host the bursty five-role review graph separately from the always-on Lightsail service. | Runtime `page47_review` is `READY` in `eu-west-2`, version 15. ARN: `arn:aws:bedrock-agentcore:eu-west-2:591697681173:runtime/page47_review-X5IwXt4Y7h`. The telemetry-enabled package is 59.2 MB compressed and 162.3 MB expanded and includes ADOT 0.19.0. The Lightsail client invoked stored Seattle matter `17394`; run `6827c1a5528448f5add0c97427dd2c6c` completed and saved a `cannot_determine` result. | The graph cannot run as the managed AWS workload or save a managed-runtime review. |
...
| OpenTelemetry | Show one review from request through the five roles. | Transaction Search is configured in `eu-west-2`; the X-Ray destination is `CloudWatchLogs/ACTIVE`, the default indexing target is 1%, and runtime version 15 includes ADOT plus the application telemetry gate. Controlled run `6827c1a5528448f5add0c97427dd2c6c` completed, but the runtime reported `localhost:4318` exporter connection refusals and its empty `spans` stream contained no searchable trace. The remaining AWS action is to enable the selected runtime's **Tracing** toggle in the AgentCore console. | The AWS execution path cannot yet be demonstrated with a searchable trace. |
...
6. Export one real post-activation review trace and retain its trace identifier. **Partial:** the telemetry-enabled runtime is deployed and a controlled review completed, but AgentCore's runtime **Tracing** toggle must be enabled in the console before the local OTLP collector will accept spans.
