...
| Amazon SESv2 | Send one review email to each matching watch. | The sender and recipient code is implemented, duplicate sends are recorded, and each message includes the private watch-management link. The 10 September 2026 VPS check found the sender parameter present, `SendingEnabled=true`, `ProductionAccessEnabled=false`, and `VerifiedForSendingStatus=true`. One temporary verified watch produced a `sent` notification with a provider `MessageId`; the watch was then deactivated. Mailbox receipt remains to be confirmed while SES stays in sandbox mode. | Residents do not receive a review without logging into the console. |
| Page 47 console service | Serve the resident console and API from the public host. | `page47-web.service` is enabled and healthy on `127.0.0.1:8090`. DNS, the `page47.xcover.online` certificate, dedicated Nginx host, HTTP redirect, HTTPS health, root response, renewal dry-run, root-relative links, legacy `/page47` redirect, public URL parameter, and controlled SES application delivery are verified. | Mailbox receipt and ongoing resident delivery still need operational confirmation. |
...
4. Configure the SES sender parameter and verify SES sender status. **Partial:** sender configuration and application delivery are complete; mailbox receipt remains to be confirmed.
