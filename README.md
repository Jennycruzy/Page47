...
The collector writes a success heartbeat only after the full scheduled run completes. A checked-in CloudWatch agent configuration tails that log and a configuration script creates the metric filter and missed-run alarm. These monitoring resources are prepared in the repository but are not live until the Lightsail instance is reachable again.
